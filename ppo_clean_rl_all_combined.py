...
class Args:
    ...
    # Impala specific arguments
    cnn_feature_dim: int = 256
    """dimension of the CNN feature vector"""
    use_maxpool: bool = True
    """whether to use maxpool in the ImpalaA blocks"""

class CropObservationWrapper(gym.ObservationWrapper):
    def __init__(self, env):
        super().__init__(env)
        # Get original observation space shape and adjust height
    ...
    def observation(self, obs):
        # Crop the bottom half of the original resolution
        height = obs.shape[0]
        if len(obs.shape) == 2:
            return obs[height//2:, :]
        else:
...
class ImpalaBlock(nn.Module):
    def __init__(self, input_channels, output_channels):
        super().__init__()
        self.conv = layer_init(nn.Conv2d(input_channels, output_channels, 3, stride=1, padding=1))
        self.res1 = ResidualBlock(output_channels)
        self.res2 = ResidualBlock(output_channels)
...
def make_env(env_id, idx, capture_video, run_name):
    def thunk():
        ...
        env = CropObservationWrapper(env)  # Crop before resizing
...
class Agent(nn.Module):
    def __init__(self, envs):
        super().__init__()
        # ImpalaA CNN architecture
        c = 16  # Initial number of channels
        
        self.network = nn.Sequential(
            # Initial convolution layer (84x84x4 -> 42x42x16)
            layer_init(nn.Conv2d(4, c, 3, stride=1, padding=1)),
            nn.MaxPool2d(3, stride=2, padding=1),
            
            # First ImpalaBlock (42x42x16 -> 21x21x32)
            ImpalaBlock(c, c*2),
            
            # Second ImpalaBlock (21x21x32 -> 11x11x32)
            ImpalaBlock(c*2, c*2),
            
            # Third ImpalaBlock (11x11x32 -> 6x6x32)
            ImpalaBlock(c*2, c*2),
            ...
            # Final dense layers
            layer_init(nn.Linear(c*2 * 6 * 6, 512)),  # 512 hidden dim
            nn.ReLU(),
        )
