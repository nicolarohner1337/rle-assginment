...
class SpaceInvadersSurvivalWrapper(gym.Wrapper):
    """Adds a small reward for survival time"""
    def __init__(self, env, survival_bonus=0.01):
        super().__init__(env)
        self.env = env
    ...
    def step(self, action):
        ...
        # Add survival bonus for non-terminal states
        # For vectorized environments, we need to handle arrays
        if isinstance(reward, (np.ndarray, list)):
            # Add bonus only where the episode isn't done
            done = np.logical_or(terminated, truncated)
            survival_bonuses = np.where(~done, self.survival_bonus, 0.0)
            reward = np.add(reward, survival_bonuses)
        else:
...
def make_env(env_id, idx, capture_video, run_name):
    def thunk():
        ...
        env = gym.wrappers.FrameStack(env, 4)
         # Add survival bonus wrapper after other wrappers
        env = SpaceInvadersSurvivalWrapper(env, survival_bonus=1)
        return env
